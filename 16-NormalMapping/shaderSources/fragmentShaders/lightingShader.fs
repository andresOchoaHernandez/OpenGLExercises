...
vec3 calculatePointLigth(PointLigth l, vec3 lp, vec3 n, vec3 fp, vec3 vp);
vec3 calculateSpotLigth(SpotLigth l, vec3 lp, vec3 ld, vec3 n, vec3 fp, vec3 vp);
...
void main(){
    ...
    vec3 pLigth = calculatePointLigth(pointLigth,tangentSpace.pointLigthPosition,normal,tangentSpace.fragmentTangentPosition,tangentSpace.viewPosition);
    vec3 sLigth = calculateSpotLigth(spotLigth,tangentSpace.flashLigthPosition,tangentSpace.flashLigthDirection,normal,tangentSpace.fragposition,tangentSpace.viewPosition);
...
vec3 calculateDirectionalLigth(DirectionalLigth l, vec3 ld, vec3 n)
{
    ...
    vec3 specular = l.specular * (pow(max(dot(n,reflectionDirection),0.0),32)); /* ADD SPECULAR TEXTURE IF ANY */
...
vec3 calculatePointLigth(PointLigth l, vec3 lp, vec3 n, vec3 fp, vec3 vp)
{
    ...
    /* SPECULAR COMPONENT */
    //vec3 viewDirection = normalize(vp - fp);
    vec3 reflectionDirection = reflect(-ligthDirection,n);
    vec3 specular = l.specular * (pow(max(dot(n,reflectionDirection),0.0),32)); /* ADD SPECULAR TEXTURE IF ANY */
...
vec3 calculateSpotLigth(SpotLigth l, vec3 lp, vec3 ld, vec3 n, vec3 fp, vec3 vp)
{
    ...
    //vec3 viewDirection = normalize(vp - fp);
    vec3 reflectionDirection = reflect(-ligthDirection,n); 
    vec3 specular = l.specular * (pow(max(dot(n,reflectionDirection),0.0),32.0)); /* ADD SPECULAR TEXTURE IF ANY */
